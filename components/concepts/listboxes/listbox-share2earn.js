      ...
      <Listbox value={selected} onChange={setSelected}>
        <div className="relative">
          <Listbox.Button className="flex items-center relative w-full py-3 pl-3 pr-10">
            <span
              class={`flex justify-center items-center w-px-32 h-px-32 rounded-full mr-4 brand--${selected.name}`}
            >
              ...
              {bannerTypes.map((bannerType, bannerTypeIdx) => (
                ...
                >
                  {({ selected, active }) => (
                    <>
                      <span
                        class={`flex justify-center items-center w-px-32 h-px-32 rounded-full mr-4 brand--${bannerType.name}`}
                      >
                        <span className="icon">{bannerType.icon}</span>
                      </span>

                      <span className="block truncate text-sm text-left">
                        <strong className={`${selected ? " " : " "} block truncate`}>
                          {bannerType.name}
                        </strong>
                        <span className="text-gray-500 dark:text-gray-400">{bannerType.desc}</span>
                      </span>
