...
const SubcribeByRIR = ({ project, accountBalance, setIsBusd }) => {
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      ...
      onApprove: () => {
        return callWithGasPrice(rirContract, "approve", [
      ...
      onConfirm: () => {
        ...
        return callWithGasPrice(launchpadContract, "createOrder", [
          ethers.utils.parseEther(numberRIR),
          ethers.utils.parseEther(numberRIR).mul(100),
        ]);
      },
  ...
  const disableBuying =
    !isApproved || isConfirmed || !numberRIR || new ethers.utils.parseEther(numberRIR).lte(0);
  return (
    <div className="global-padding">
      <div className="mb-4">
        <label for="price" className="block text-xs font-medium uppercase mb-2">
          You pay
        </label>
        <div className="mt-1 relative rounded-md shadow-sm">
          <input
            ...
            name="numberRIR"
            id="numberRIR"
            className="inputbox inputbox-lg inputbox-numbers !pr-20"
            placeholder="Number RIR"
            value={numberRIR}
            onChange={(e) => {
              setNumberRIR(e.target.value);
            }}
          />
          <div className="absolute inset-y-0 right-0 flex items-center">
            ...
            <select
              id="currency"
              name="currency"
              className="focus:ring-primary-500 focus:border-primary-500 h-full py-0 pl-2 pr-7 border-transparent bg-transparent rounded-md text-sm"
            >
              <option>RIR</option>
            </select>
          </div>
      ...
      <div className="mt-8">
        <button
          className={
            "btn btn-default btn-default-lg btn-purple mr-2" +
        ...
        <button
          className={
            "btn btn-default btn-default-lg btn-purple" + (disableBuying ? " disabled" : "")
          }
          id="swap-button"
          width="100%"
          scale="md"
          onClick={(e) => {
            handleConfirm(), setNumberRIR(0);
          }}
        >
          Confirm
        </button>
      ...
      {accountBalance?.busdBalance > 0 && (
        <div className="mt-2">
          <button
            ...
            className="btn btn-default btn-default-lg w-full"
            disabled=""
            id="swap-button"
            width="100%"
            scale="md"
...
const SubcribeByBUSD = ({ project, accountBalance, setIsBusd }) => {
  ...
  return (
    <div className="global-padding">
      ...
      <div className="mt-8">
        <button
          className="btn btn-default btn-default-lg w-full btn-purple"
          disabled=""
          id="swap-button"
          width="100%"
          scale="md"
