...
const PostNotice = observer(({ type, setTabCallback }) => {
  ...
  let startString =
    type == "news"
      ? ` ${t("post notice start")}
  <a href="${source.url}" rel="nofollow noreferrer" target="_blank" className="link ml-1">
  ...
  <a href="${source.url}" rel="nofollow noreferrer" target="_blank" className="link ml-2">
    <strong>${source.name}</strong> 
    <span className="icon ml-1"><i className="fa-duotone fa-external-link text-2xs relative -top-0.5"></i></span>
  </a>`;
  let about =
